...
class KeychainManager: CredentialStoreProtocol {

    private let tag = "com.trudev.Punch"
    private func setupQueryDictionary(forKey key: String) -> [String: Any] {
        ...
        let fullKey = "\(tag).\(key)"
        queryDictionary[kSecAttrAccount as String] = fullKey.data(using: .utf8)
    ...
    public func set(string: String, forKey key: String) throws {
        ...
        do {
            ...
            switch error {
            case KeychainManagerError.failure(status: let status):
